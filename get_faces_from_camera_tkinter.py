...
class Face_Register:
    ...
    def GUI_clear_data(self):
        ...
        self.log_all["text"] = "Known faces and `features_all.csv` removed!"
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info, text="Faces known : ").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        self.label_cnt_face_in_database.grid(row=2, column=1, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="Faces in frame : ").grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_face_cnt.grid(row=3, column=2, columnspan=3, sticky=tk.W, padx=5, pady=2)

        self.label_warning.grid(row=4, column=0, columnspan=3, sticky=tk.W, padx=5, pady=2)

        tk.Button(self.frame_right_info,
                  text='Delete all previours data',
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="Enter name").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Take Image ").grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

        tk.Button(self.frame_right_info,
                  text='Take Image ',
                  command=self.save_current_face).grid(row=10, column=0, columnspan=6, sticky=tk.W)
        def des():
            sys.exit()
        tk.Button(self.frame_right_info,
                  text='EXIT',
                  command=des).grid(row=12, column=3, columnspan=4, sticky=tk.W)
    ...
    def save_current_face(self):
        if self.face_folder_created_flag:
            ...
            self.log_all["text"] = "Please run another step"
